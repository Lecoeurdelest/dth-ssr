...
export function BookServiceModal({ isOpen, onClose, serviceName, serviceId }: BookServiceModalProps) {
  ...
  const [currentStep, setCurrentStep] = useState<'form' | 'worker' | 'payment'>('form');

  const handleInputChange = (field: keyof BookingFormData, value: string) => {
    setFormData(prev => ({ ...prev, [field]: value }));
  };

  const handleNextToWorker = () => {
  ...
  const handleWorkerSelected = (workerId: string) => {
    setFormData(prev => ({ ...prev, selectedWorkerId: workerId }));
    ...
    <>
      <Dialog open={isOpen && currentStep === 'form'} onOpenChange={onClose}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleNextToWorker(); }} className="space-y-5 mt-4">
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    onChange={(e) => handleInputChange('contactMethod', e.target.value as 'phone' | 'email')}
      ...
      {showPayment && (
        <PaymentSection
          isOpen={showPayment}
          onClose={handleBackToWorker}
          bookingSummary={mockBookingSummary}
          selectedWorkerId={formData.selectedWorkerId}
          onPaymentMethodSelected={(method) => handleInputChange('paymentMethod', method)}
